...
function gainImg(url,typeName,grade,i){//添加图片
  ...
  $.ajax({
    ...
    success: function(data){//所有图片都放在了一个网址，无法根据品种来获得图片
      if(i==0){
        selector = ".option" + i;
        $(selector).find(".showing1").attr("src",data.image_1);
        $(selector).find(".showing0").attr("src",data.image_2);
        $(selector).find(".showing2").attr("src",data.image_3);
      }
      else if(i==1){
        selector = ".option" + i;
        $(selector).find(".showing0").attr("src",data.image_1);
        $(selector).find(".showing1").attr("src",data.image_2);
        $(selector).find(".showing2").attr("src",data.image_3);
      }
      else if(i==2){
          selector = ".option" + i;
